...
public class PostMonthModelImplementation implements PostMonthModel {
    Context context;

    public PostMonthModelImplementation(Context context) {
        this.context = context;
    }
    @Override
    public void postMostMonthRequest(PostMonthRequest postMonthRequest, RequestCompleteListener<ResponseBody> requestCompleteListener) {
        Api api = RetrofitClient.getClient().create(Api.class);
        api.setPostMonth(postMonthRequest).enqueue(new Callback<ResponseBody>() {
            @Override
